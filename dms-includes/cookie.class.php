...
class CacheCookie {
    ...
    static function set($key, $value, $lifetime)
    {
        ...
        $cookie                     = hash_hmac(CACHE_COOKIE_DIGEST_METHOD, $cookie_json, CACHE_COOKIE_SECRET_KEY) . '|' . $cookie_json;
        self::_wipe_previous_cookie(CACHE_COOKIE_NAME);
        setcookie(CACHE_COOKIE_NAME, $cookie, $now + CACHE_COOKIE_DURATION, COOKIES_PATH, COOKIES_DOMAIN, FALSE, TRUE);
    ...
    static function delete($key)
    {
        ...
        $cookie                     = hash_hmac(CACHE_COOKIE_DIGEST_METHOD, $cookie_json, CACHE_COOKIE_SECRET_KEY) . '|' . $cookie_json;
        self::_wipe_previous_cookie(CACHE_COOKIE_NAME);
        setcookie(CACHE_COOKIE_NAME, $cookie, time() + CACHE_COOKIE_DURATION, COOKIES_PATH, COOKIES_DOMAIN, FALSE, TRUE);
        $_COOKIE[CACHE_COOKIE_NAME] = $cookie;
    ...
    {
        self::_wipe_previous_cookie(CACHE_COOKIE_NAME);
        setcookie(CACHE_COOKIE_NAME, '', 1, COOKIES_PATH, COOKIES_DOMAIN, FALSE, TRUE);
